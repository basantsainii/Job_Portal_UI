...
function UserProfile() {
  ...
  const userProfileData = (Resume) => {
    if (Resume) {
      const resumeName = decodeURIComponent(Resume.split("/").pop().split("-")[1]);
  ...
  const FetchUserProfile = async () => {
    ...
    try {
      const res = await axios.get("http://localhost:3000/api/fetch-user-profile", {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 md:p-8 bg-slate-50">
      {
        !profileData ? <div class="mx-auto w-full max-w-sm rounded-md border border-blue-300 p-4">
      ...
      <div className="bg-white shadow-md rounded-xl p-4 sm:p-6 flex flex-col sm:flex-row items-center gap-6">
        ...
        >
          {!profileData?.dp ? (
            ...
            <img src={profileData?.dp} alt="Profile" className="w-full h-full object-cover" />
          )}
        </div>
        <div className="text-center sm:text-left">
          <h2 className="text-2xl font-semibold">{profileData?.fname} {profileData?.lname}</h2>
          ...
          <p className="text-gray-600 flex items-center justify-center sm:justify-start gap-2 mt-2">
            <i className="fa-solid fa-envelope text-blue-800"></i> {profileData?.email}
          </p>
          <p className="text-gray-600 flex items-center justify-center sm:justify-start gap-2">
            <i className="fa-solid fa-phone text-blue-800"></i> {profileData?.phone}
          </p>
        </div>
      </div>
      }
      ...
      <div className="bg-white shadow-md rounded-xl p-4 sm:p-6 mt-6">
        <div className="flex justify-between">
          ...
          <p className="text-blue-800 cursor-pointer" onClick={() => setActiveModal("resume")}>Update</p>
        </div>
        <div className="mt-4">
          {profileData?.resume ? (
            <div className="flex flex-col sm:flex-row justify-between items-center border p-3 rounded-lg">
              ...
              <div className="flex gap-4 mt-2 sm:mt-0">
                <a href={profileData?.resume} download={userData} className="text-blue-800">
            ...
            <p className="text-gray-400 text-center sm:text-left">Upload your resume for employers</p>
          )}
        </div>
      </div>
      ...
      <div className="bg-white shadow-md rounded-xl p-4 sm:p-6 mt-6">
        ...
        <div className="mt-4 grid gap-4">
          ...
          <div className="bg-gray-100 p-4 rounded-lg shadow-sm flex flex-col sm:flex-row justify-between items-start">
            <div>
              <h4 className="font-medium">Master's in Software Engineering</h4>
              <p className="text-gray-600">ABC Institute</p>
              <p className="text-gray-500 text-sm">2022 - 2024 | Online</p>
            </div>
            <i className="fa-solid fa-pen-to-square text-blue-800 cursor-pointer"></i>
          </div>
        </div>
      ...
      <div className="bg-white shadow-md rounded-xl p-6 mt-6">
        ...
        <div className="mt-4 grid grid-cols-2 gap-4 text-gray-600">
          ...
          <p><span className="font-medium">Date of Birth:</span> 13 August</p>
          <p><span className="font-medium">Languages:</span> Hindi, English</p>
          <p><span className="font-medium">Address:</span> Saharanpur</p>
        </div>
      </div>
    </div>
  );
